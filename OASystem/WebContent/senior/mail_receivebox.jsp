...
<ul class="nav nav-tabs ">
        <li role="presentation" class="active"><a href="../s_Mail?method=mail_receivebox">收信箱</a></li>
        <li role="presentation"><a href="../s_Mail?method=mail_sendbox">已发送</a></li>
        <li role="presentation"><a href="../s_Mail?method=addemployee">新发邮件</a></li>
 </ul>
  ...
  <section class="content">
    <div class="row">
        <div class="col-md-12">
          ...
          <c:choose>
          <c:when test="${sessionScope.receive==null || fn:length(sessionScope.receive) == 0}">  
          <center>  
          <strong>还没有人给您发送邮件！</strong>  
          </center>  
          </c:when>  
          <c:otherwise>
          <form action="../s_Mail?method=deletereceive" method="post">
            <div class="box-header with-border">
             
              <h3 class="box-title">收信箱</h3>
            ...
            <div class="box-body no-padding">
              <div class="mailbox-controls">
                ...
                <!-- /.btn-group -->
                <button type="button" class="btn btn-default btn-sm"><a href="../s_Mail?method=mail_receivebox" ><i class="fa fa-refresh"></i></a></button>
                </div>
                <!-- /.pull-right -->
              </div>
               <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped " style='table-layout:fixed;'>
                  <tbody>
                  <c:forEach items="${sessionScope.receive}" var="item" varStatus="status">  
                  <tr>
                    <td style="width:5%;"><input type="checkbox" name="receiveID" value="${item.receiveID }" ></td>
                    <td style="width:20%;" class="mailbox-name" ><a href="../s_Mail?method=mail_receivedetail&receiveID=${item.receiveID }">${item.receiveTitle }</a></td>
                    <td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;width:40%;" class="mailbox-subject">
                    ${item.receiveText }
                    </td>
                    <td class="mailbox-attachment" style="width:10%;">${item.fromName }</td>
                    <td class="mailbox-date" style="width:15%;">${item.receiveDate }</td>
                    <td class="mailbox-date" style="width:10%;">${item.read }</td>
                  </tr>
                  </c:forEach>
                  </tbody>
                  </table>
