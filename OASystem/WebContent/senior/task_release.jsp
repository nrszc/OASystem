...
<div class="wrapper">
 ...
 <!-- Content Header (Page header) -->
      <ul class="nav nav-tabs">
        <li role="presentation" class="active"><a href="../s_Task?method=myReleaseTask">我发起的</a></li>       
         <li role="presentation"><a href="../s_Task?method=employee">新增任务</a></li>
      </ul>
    <!-- Content Header (Page header) -->
   ...
   <section class="content">
      <!-- Info boxes -->
       ...
       <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
        <c:choose>
    <c:when test="${sessionScope.myReleaseTask==null || fn:length(sessionScope.myReleaseTask) == 0}">  
    <center>  
    <strong>还没有发布过任务的数据哦！请点击新增任务来添加吧~</strong>  
    </center>  
    </c:when>  
    <c:otherwise>
	<table class="table table-hover table-bordered" id="table">
		...
		<tbody>
		<c:forEach items="${sessionScope.myReleaseTask}" var="item" varStatus="status">  
		<tr>
		    <td><a href="../s_Task?method=task_detail&taskID=${item.taskID }">${item.taskName}</td>
		    <td>${item.taskInfo}</td>		     
		    <td>${item.taskTime }</td>
		    ...
		    <td>
		    <a href="../s_Task?method=deletetask&taskID=${item.taskID }">删除</a>
		    </td>
	    </tr>
	    </c:forEach>
   	    </tbody>
