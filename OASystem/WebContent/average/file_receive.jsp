    ...
    <!-- /.sidebar -->
		...
		<div class="content-wrapper">
			...
			<ul class="nav nav-tabs ">
				...
				<li role="presentation"><a href="../a_File?method=showemployee">发送文件</a></li>
			</ul>
					...
					<div class="col-xs-12">
						<div class="box">
							...
							<div class="box-body">
								<c:choose>
									<c:when
										test="${sessionScope.mf==null || fn:length(sessionScope.mf) == 0}">
										<center>
											<strong>还没有同事给您发送文件哦~</strong>
										</center>
									</c:when>
									<c:otherwise>
										<table class="table table-hover table-bordered" id="table">
											...
											<tbody>
												<c:forEach items="${sessionScope.mf}" var="item"
													varStatus="status">
													<tr>
														<td>${item.fileName }</td>
														<td>${item.fileText }</td>
														...
														<td>${item.fileFrom }</td>
														<td><a
															href="../a_File?method=download&fileName=${item.fileName }">下载文件</a></td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
