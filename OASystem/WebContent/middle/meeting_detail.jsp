...
<div class="wrapper">
 ...
 <!-- Content Header (Page header) -->
      <ul class="nav nav-tabs">
        <li role="presentation"><a href="../m_Meeting?method=myMeeting">我参与的</a></li>
        <li role="presentation"><a href="../m_Meeting?method=myRelease">我发起的</a></li>
        <li role="presentation"><a href="../m_Meeting?method=employee">新增会议</a></li>
      </ul>
    <!-- Content Header (Page header) -->
   ...
   <section class="content">
      <!-- Info boxes -->
       ...
       <div class="col-xs-12">
		...
		<tbody>
		<tr>
		    <td>会议名称</td><td>${sessionScope.meeting.meetingName}</td>
		</tr>
		<tr>
		    <td>会议描述</td><td>${sessionScope.meeting.meetingText}</td>
		</tr>	
		<tr>	     
		    <td>会议地点</td><td>${sessionScope.meeting.hall }</td>
		</tr>  
		<tr>
		    <td>开始时间</td><td>${sessionScope.meeting.startTime}</td>
		</tr>
		<tr>
		    <td>结束时间</td><td>${sessionScope.meeting.endTime}</td>
		</tr>
		<tr>
		    <td>发布人</td><td>${sessionScope.meeting.employeeID}</td>
		</tr>
		<tr>
		    <td>发布时间</td><td>${sessionScope.meeting.createTime}</td>
		</tr>
		<tr>
		    <td>参与人员</td><td>${sessionScope.meeting.isPart}</td>
		</tr>
		<tr>
		    <td>任务状态</td>
		    <td>
		    <c:if test="${sessionScope.meeting.condition=='进行中' }">
		    <span class="label label-primary">${sessionScope.meeting.condition}</span>
		    </c:if>
		     <c:if test="${sessionScope.meeting.condition=='未开始' }">
		    <span class="label label-success">${sessionScope.meeting.condition}</span>
		    </c:if>
		     <c:if test="${sessionScope.meeting.condition=='已结束' }">
		    <span class="label label-default">${sessionScope.meeting.condition}</span>
		    </c:if>
		    </td>
		</tr>
   	    </tbody>
