...
<div class="wrapper">
 ...
 <!-- Content Header (Page header) -->
      <ul class="nav nav-tabs">
        ...
        <li role="presentation" class="active"><a href="../m_Meeting?method=myRelease">我发起的</a></li>
   ...
   <section class="content">
      <!-- Info boxes -->
       ...
       <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
        <c:choose>
    <c:when test="${sessionScope.myRelease==null || fn:length(sessionScope.myRelease) == 0}">  
    <center>  
    <strong>还没有相关会议的数据哦~</strong>  
    </center>  
    </c:when>  
    <c:otherwise>
	<table class="table table-hover table-bordered" id="table">
		...
		<tbody>
		<c:forEach items="${sessionScope.myRelease}" var="item" varStatus="status">  
		<tr>
		    <td><a href="../m_Meeting?method=meeting_detail&meetingID=${item.meetingID }">${item.meetingName}</a></td>
		    <td>${item.meetingText}</td>
		     <td>${item.hall}</td>
		    ...
		    <td>${item.createTime }</td>
		     <td>
		      <c:if test="${item.condition=='进行中' }">
		    <span class="label label-primary">${item.condition}</span>
		    </c:if>
		     <c:if test="${item.condition=='未开始' }">
		    <span class="label label-success">${item.condition}</span>
		    </c:if>
		     <c:if test="${item.condition=='已结束' }">
		    <span class="label label-default">${item.condition}</span>
		    </c:if>
		     </td>
		     <td>
		     <a href="../m_Meeting?method=meeting_delete&meetingID=${item.meetingID }">删除</a>
		     </td>	
	    </tr>
	    </c:forEach>
   	    </tbody>
