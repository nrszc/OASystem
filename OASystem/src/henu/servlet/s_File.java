...
/**
 * Servlet implementation class s_File
 */
@WebServlet("/s_File")
public class s_File extends HttpServlet {
	...
	private void showemployee(HttpServletRequest request, HttpServletResponse response) throws IOException {
		s_FileIF ms = s_Factory.getFileInstance();
	...
	private void uploadfile(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		s_FileIF ms = s_Factory.getFileInstance();
		//上传公司文件
		if(ftype.equals("company")) 
		{
			CompanyFile cf = new CompanyFile();
			cf.setFileFrom(id);
			cf.setFileTime(ftime);
			cf.setFileName(fname);
			cf.setFileText(ftext);
			boolean result =   ms.saveCompanyfile(cf);
			if(result){
	...
	private void file_send(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		PrintWriter   out   =   response.getWriter(); 
		out.println("<script>window.location.href='/OASystem/senior/file_send.jsp'</script>");
	}
	...
	private void file_receive(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		PrintWriter   out   =   response.getWriter(); 
		out.println("<script>window.location.href='/OASystem/senior/file_receive.jsp'</script>");
	}
