...
public class s_FileImpl implements s_FileIF{
	...
	@Override
	public boolean saveCompanyfile(CompanyFile cf) {
		String sql = "insert into companyfile(fileTime,fileText,fileFrom,fileName) values(?,?,?,?)";
		PreparedStatement ps = DbcpPool.executePreparedStatement(sql);
		int r=0;
		try {
			ps.setString(1, cf.getFileTime());
			ps.setString(2, cf.getFileText());
			ps.setString(3, cf.getFileFrom());
			ps.setString(4, cf.getFileName());
			r=ps.executeUpdate();
			ps.close();
		}catch(SQLException e)
		{
